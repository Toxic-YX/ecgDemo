...
- (void)setECGViews{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        [ecgView setFrame:CGRectMake(5, 40 + kMartin10 * (i + 1)  + i *(kmm * 4) , kScrenWidth - 10, kmm * 4)];
        ecgView.alpha = 1;
...
- (void)popData{
    NSMutableArray *testArr = [NSMutableArray arrayWithCapacity:10];
    for (int i = _index; i <_index + _drawNumbers; i ++) {
        [testArr addObject:_dataArr[i]];
    }
    _index+=7;
    if (_index > 2330) {
        _index = 0;
    ...
    view.pointArray = testArr;
//    传递数据
    
    
     if (testArr.count > _drawNumbers - 1) {
        [view fireDrawing];
    } else {
//            [view3 keepDrawing];
//            [view2 keepDrawing];
...
- (NSMutableArray *)ecgViewsArr{

    if (!_ecgViewsArr) {
        _ecgViewsArr = [[NSMutableArray alloc]init];
    }
    return _ecgViewsArr;
}
